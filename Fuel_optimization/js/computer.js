$(document).ready(function() {
	...
	function showPosition(position)
	{
		...
		$.ajax({
			...
			success:function(data) {
				...
				$("#dataout").append("<tr><td>"+sno+"</td><td>"+addr.formattedAddress+"</td><td>"+lat+"N, "+lng+"E</td><td><button class='btn btn-success btn-sm' id='src' data-record="+sno+"> Choose</button><td></tr>");
	...
	function getLocation()
	{
		...
		{
			alert("Cannot fetch location of your device");
		}
	}
		...
		if(selectedloc.length>0)
		{
			$.ajax({
				...
				success: function(data)
				{
					...
					$("#dataout").append("<tr><td>"+sno+"</td><td>"+selectedloc+"</td><td>"+latlng[0]+"N,"+latlng[1]+"E</td><td><button class='btn btn-success btn-sm' id='src' data-record="+sno+"> Choose</button><td></tr>");
	...
	$(document).on("click","#src",function() {
		...
		for(var i=0;i<sno;i++)
		{
			content+="<strong class='text-success'>"+Number(i+1)+". "+locnames[finalpath[i]]+"</strong>  &#10145;<br/>";
		}
		var totalcost=0
		var j=0,k=0
		while(j<sno-1)
		...
		content=content.substr(0,content.length-13);
